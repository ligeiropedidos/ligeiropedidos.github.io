...
const avisar = (pag, token) => worker.fetch(new Request('https://w/', { method: 'POST', headers: { 'asaas-access-token': token === undefined ? env.ASAAS_WEBHOOK : token, 'Content-Type': 'application/json' }, body: JSON.stringify({ event: 'PAYMENT_RECEIVED', payment: pag }) }), env);
